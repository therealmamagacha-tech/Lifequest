...
ZONE_BADGES = {
    "cuisine": ("🍳", "badge_cuisine", "#ff6b35"),
    "sol":     ("🧹", "badge_sol",     "#00f2ff"),
    "sanitaires": ("🚿", "badge_sanitaires", "#7b2fff"),
    "general": ("⚙️", "badge_general", "#ffffff"),
}
...
if palmares:
    ...
    for idx, (zone_key, (icon, label_key, color)) in enumerate(ZONE_BADGES.items()):
    ...
    for idx, (badge_id, icon, label_key, condition) in enumerate(BADGES_DEF):
        ...
        with ach_cols[idx % 3]:
            ...
            st.markdown(f'''
                <div class="achievement-badge" style="opacity:{opacity}; border-color:{border_color}; box-shadow:{glow};">
                    ...
                    <p style="font-family:Orbitron; font-size:0.62rem; color:{"#ffb700" if unlocked else "#555"}; margin:4px 0 0; letter-spacing:0.5px;">{label}</p>
                </div>
            ''', unsafe_allow_html=True)
    ...
    for entry in sorted_pal:
        ...
        diff_icons = {"facile": "🟢", "easy": "🟢", "moyen": "🟡", "medium": "🟡", "difficile": "🔴", "hard": "🔴"}
